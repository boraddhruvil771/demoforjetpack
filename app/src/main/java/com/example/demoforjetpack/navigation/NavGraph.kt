package com.example.demoforjetpack.navigation
...
import com.example.demoforjetpack.screens.HomeScreen
import com.example.demoforjetpack.screens.LoginScreen
import com.example.demoforjetpack.screens.ProfileScreen
import com.example.demoforjetpack.screens.SearchScreen

@Composable
fun NavGraph(navController: NavHostController) {

    NavHost(
        navController = navController, startDestination = NavRoute.Login.path
    ) {
            ...
            navigateToProfile = { id, showDetails ->
                navController.navigate(
                    NavRoute.Profile.withArgs(
                        id.toString(), showDetails.toString()
                    )
                )
            },
            navigateToSearch = { query ->
                navController.navigate(NavRoute.Search.withArgs(query.toString()))
            },
...
private fun addSearchScreen(
    ...
    navGraphBuilder.composable(
        ...
        SearchScreen(query = args?.getString(NavRoute.Search.query),
            popBackStack = { navController.popBackStack() },
            popUpToLogin = { popUpToLogin(navController) })
    }
}
